import React, { useState } from "react";

const Pokedesk = () => {
  const [pokemon, setPokemon]=useState({
    name:'Nome do pokemon',
    type:'Tipo do Pokemon',
    descrition:'Pokemon',
    coup:'golpe',
    img:"https://www.ensino.eu/media/3kyh4vko/pokemon.jpg",
    number: 1
 
  })
  const handleClick = ()=>setPokemon(pokemon.number + 1)
  return (
   <> 
    <div className="center">
      <div className="principal-container">
        <div className="information">
          <section className="container-pokemon">
            <header className="pokemon">
              <h1 className="name">{pokemon.name}</h1>
              <h1>{pokemon.type}</h1>
            </header>
          </section>

          <section className="img">
            <img className="img-pokemon" src={pokemon.img} />
          </section>
          <div className="descrition">
            <section className="descrition">
              <p className="pokemon-descrition">{pokemon.descrition}</p>
            </section>
            <section className="golpes">
              <p className="golpes-descrition">{pokemon.coup}</p>
            </section>
          </div>
        </div>
      </div>
    </div>
    <div className="arrow">
        <img onClick={handleClick}  className="arrow-seta-direita" src="data:image/png;base64," alt="" />
        <p className="pokemon-number">{pokemon.number}</p>
        <img onClick={handleClick} className="arrow-seta-esquerda" src="data:image/png;base64," alt="" />
    </div>
   </> 
    
  );
};

export default Pokedesk;
